    ...
    var Home = Backbone.View.extend({
        ...
        initData: function () {
            userTable = $('#table').DataTable({
                "ajax": {
                    url: baseUrl + "/loanPlan/tables"
                },
                "columns": [
                    {"data": "financeSolutionName"},
                    {"data": "actualAnnual"},
                    {"data": "earlyStage"},
                    {"data": "laterStage"},
                    {"data": "earlyRate"},
                    ...
                    {"data": "addedRate"},
                    {
                        render: function (data, type, row, meta) {
                            var str = "";
                            if ($.inArray("/loanPlan-put", resourceData) > -1) {
                                str += "<button  data-text='编辑金融方案'  data-id='addUser' data-link='../addLoanPlan/addLoanPlan.html?id=" + row.id + "' class='btn btn-primary edit-btn btn-xs margin-right-5'><i class='fa fa-pencil' aria-hidden='true'></i> 编辑</button>"
                            }

                            if ($.inArray("/loanPlan/editPwd-post", resourceData) > -1) {
                            ...
                            if ($.inArray("/loanPlan/*-delete", resourceData) > -1) {
        ...
        handlerSureDel: function () {
            var _this = this;
            utils.getDelete("/loanPlan/" + resourceId, {}, function (res) {
                utils.showTip("删除成功");
                setTimeout(function () {
